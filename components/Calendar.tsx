        ...
        <div key={day} className={dayClasses} onClick={() => onDateChange(date)}>
          ...
          <div className="mt-1 overflow-hidden flex-grow space-y-0.5">
            {dailyTasks.slice(0, 2).map(task => {
              const tagColorClasses: Record<TaskTag, string> = {
              ...
              return (
                <div key={task.id} className={`text-xs truncate px-1.5 py-0.5 rounded font-medium ${tagColorClasses[task.tag] || tagColorClasses.work}`}>
                  {task.title}
                </div>
              );
            })}
