...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedTasks) {
        // Support old tasks that don't have a tag or recurring property.
  ...
  const handleTouchEnd = () => {
    ...
    if (swipeDistance > 50) { // Swiped left
      ...
      else if (view === 'report') setView('tennis');
    } else if (swipeDistance < -50) { // Swiped right
      if (view === 'tennis') setView('report');
      else if (view === 'report') setView('today');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      >
        <div 
          className="flex w-[400%] h-full transition-transform duration-300 ease-in-out"
          style={{ transform: `translateX(${getTransformValue(view)})` }}
        >
          <div className="w-1/4 px-2 lg:px-4">
